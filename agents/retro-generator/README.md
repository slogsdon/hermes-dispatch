...
A retro is genuine synthesis, reading messy notes, separating symptom from root cause, and
deriving a few durable changes. That's the hard, long-context step the brief reserves
`quality` (qwen3.6:35b-mlx) for (same reasoning as `vault-distiller`). The model's thinking
is on a separate channel, so output stays clean under the project config's
...
- Shares the 22 GB `quality` slot with `vault-distiller` / `gtm-planner` / `lead-designer`;
  run it alone, don't co-resident two large models on the 32 GB box.
