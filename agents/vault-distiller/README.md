...
through the `obsidian` skill/CLI per a rules):
...
This is the one agent that earns `max`. The brief's rule: reserve
the `max` tier for the **genuinely hard, long-context step** and use cheap models
elsewhere. Note synthesis, collapsing a sprawling note into durable atomic claims and
inferring the right links, is exactly that step. It runs *alone*; don't co-resident it with another large model.

## Caveats

- is MoE/long-context; if you feed a very long note and see swapping, lower the
