...
Same call as `triage-router`/`invoice-tracker`. The obvious pick is `fast`, but it's a **thinking** model that streams chain-of-thought before
the JSON even with `-Q`, and its 1.2B reasoning mis-picks fields. So this agent uses
**`structured`**, explicitly "fast structured tasks, *no thinking*", for clean
...
- Want sub-second triage on small batches? Switch to `alias: fast`, `parse_last_line`
 is already on to absorb its CoT output.
- Keep batches under the real `structured` num_ctx (~32K). For a giant inbox, chunk it.
