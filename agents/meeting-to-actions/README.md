...
not field extraction, so it runs on `analyze` (deepseek-r1:14b), the reasoning slot, rather
than the `pipeline` structured-output model the JSON agents use.

> **deepseek-r1 in the harness:** it used to fail because Hermes sent a tools array its
> Ollama template rejects. The project home now disables **all** toolsets, so zero tools are
> sent and `analyze` runs cleanly, verified. See the top-level README's tool-compatibility
...
- Keep transcripts under the real `analyze` num_ctx (~40K). For a long all-hands, summarize
  in chunks and re-run on the concatenated summaries.
- Want a faster, lighter pass that skips the reasoning? Switch to `alias: write` (gpt-oss),
