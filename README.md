...
- Stays portable. Every agent names an alias (`quality`, `code`, `pipeline`), never a model. Point those 8 aliases at MLX, CUDA, cloud, or a single model. Same agents, any backend.
...
Every agent maps to one of 8 aliases. The alias is the cost and quality tier. What backs it is your choice. See [docs/model-configuration.md](docs/model-configuration.md).

| Agent | Purpose | Alias |
|-------|---------|-------|
| [`blog-drafter`](agents/blog-drafter/) | First-draft long-form blog prose from an outline | `write` |
| [`social-media-marketer`](agents/social-media-marketer/) | Topic or draft to platform-tailored LinkedIn and X posts | `chat` |
| [`status-update-writer`](agents/status-update-writer/) | done/blocked/next bullets to a client status update | `write` |
| [`devrel-sample`](agents/devrel-sample/) | API plus capability description to a developer code sample | `code` |
| [`pr-reviewer`](agents/pr-reviewer/) | Unified diff to a five-axis code review | `quality` |
| [`seo-reviewer`](agents/seo-reviewer/) | Page or draft to a qualitative on-page SEO and AEO audit | `quality` |
| [`seo-tester`](agents/seo-tester/) | Page to machine-readable pass/fail SEO checks (JSON) | `pipeline` |
| [`gtm-planner`](agents/gtm-planner/) | Product brief to a decision-ready go-to-market plan | `quality` |
| [`gtm-executor`](agents/gtm-executor/) | GTM plan to paste-ready launch assets | `write` |
| [`lead-designer`](agents/lead-designer/) | Brief to design direction or critique (text specs) | `quality` |
| [`prospect-researcher`](agents/prospect-researcher/) | Company or contact to a sales research brief | `quality` |
| [`discovery-prep`](agents/discovery-prep/) | Prospect plus goal to a pre-call discovery plan | `quality` |
| [`proposal-writer`](agents/proposal-writer/) | Scope brief to a structured client proposal | `write` |
| [`followup-drafter`](agents/followup-drafter/) | Last interaction to a follow-up email | `write` |
| [`scope-creep-detector`](agents/scope-creep-detector/) | Agreed scope plus new request to an in/out call | `analyze` |
| [`contract-reviewer`](agents/contract-reviewer/) | Contract or vendor agreement to a severity-ranked risk pass | `quality` |
| [`tos-reviewer`](agents/tos-reviewer/) | Terms of service to a risk pass before you agree | `quality` |
| [`privacy-checker`](agents/privacy-checker/) | Privacy policy or DPA to a data map plus gaps | `quality` |
| [`cashflow-summarizer`](agents/cashflow-summarizer/) | Transactions to a weekly cashflow summary | `analyze` |
| [`invoice-tracker`](agents/invoice-tracker/) | Invoice email to vendor, amount, due date, and status | `pipeline` |
| [`expense-classifier`](agents/expense-classifier/) | Transaction descriptions to tax-relevant categories | `pipeline` |
| [`triage-router`](agents/triage-router/) | One item to a routing decision (minified JSON) | `pipeline` |
| [`inbox-triage`](agents/inbox-triage/) | Batch of emails to a classified enum plus urgency | `pipeline` |
| [`vault-distiller`](agents/vault-distiller/) | Long note to atomic concepts, `[[wikilinks]]`, and tags | `quality` |
| [`decision-journal`](agents/decision-journal/) | A decision to a durable note written for future-you | `analyze` |
| [`meeting-to-actions`](agents/meeting-to-actions/) | Notes or transcript to decisions plus action items | `analyze` |
| [`retro-generator`](agents/retro-generator/) | Project notes to a structured retrospective | `analyze` |
| [`weekly-review-synthesis`](agents/weekly-review-synthesis/) | A week of notes to a candid weekly review | `quality` |
